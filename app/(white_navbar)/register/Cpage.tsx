...
export default function Cpage({}: {}) {
  const { globalState, setGlobalState, SignInWithGoogle }: GlobalContextType =
    useGlobal?.()!
  ...
  async function SubmitRegisterUser(
    ...
    if (!email) {
      ...
      if (res.data && 'user' in res.data) {
        ...
        toast.error(
          res.error?.data.msg! ||
            'Something went wrong when try to register your account'
        )
        setLoading(false)
      }
    }
  ...
  async function handleGoogleRegister() {
    ...
    if (data && data.user.email && data.user.uid) {
      ...
      if (res.data && 'user' in res.data) {
        ...
        toast.error(
          res.error?.data.msg! ||
            'Something went wrong when try to login to your account'
        )
        setLoading(false)
      }
    }
  ...
  return (
    <>
      <div
        id="register-page"
        className="w-full min-h-[calc(100vh-4.5rem)] bg-gray-100  py-12 flex justify-start items-center   "
      >
        <Circular loading={loading} />
        {/* w-[30rem] h-[39rem] */}
        <div className="w-full h-auto sm:w-[30rem] px-4 py-8 sm:px-12  min-[250px]:mx-4 sm:mx-auto bg-white rounded-lg shadow-sm flex flex-col  gap-6">
          <h5 className="text-3xl font-semibold mb-2 text-gray-800 text-center">
            Register account
          </h5>
