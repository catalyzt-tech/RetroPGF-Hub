...
export default function Cpage({
  ...
  const accessCheckerCookieValue: string | undefined =
    Cookies.get('accessChecker')
  ...
  async function handleFilterProject(
    type: string
  ): Promise<FullProjectResNoComment[] | undefined> {
    setLoading(true)
    ...
    if (res.data && 'project' in res.data) {
      ...
      toast.error(
        res.error?.data.msg! || 'Something went wrong when try to get projects'
      )
      setLoading(false)
      return undefined
    }
  ...
  async function handleFavoriteProject(projectId: string) {
    if (
      ...
      typeof accessCheckerCookieValue !== 'string'
    ) {
      ...
      if (res.data && 'opera' in res.data) {
        ...
        toast.error(
          res.error?.data.msg! ||
            'Something went wrong when try to added or removed favorite project'
        )
      }
    }
  }
        ...
        >
          ...
          <div className="flex flex-wrap md:flex-nowrap gap-3 items-center w-full">
            <div className="hidden md:flex gap-3 items-center">
              <SortByBtn state={state} setState={setState} />

              <FilterBtn state={state} setState={setState} />
            </div>
            ...
            <div className="flex max-[330px]:flex-wrap gap-4 w-full h-full relative">
              <div className="w-full flex gap-4 min-w-32 flex-grow">
                ...
                >
                  <Search
                    size={24}
                    className=" pl-0.5 z-20 fill-white rounded-full "
                  />
                </div>
              </div>
          ...
          {loading ? (
            <div role="status" className="flex justify-center mt-12">
              <svg
                aria-hidden="true"
                className="w-8 h-8 text-gray-200 animate-spin dark:text-gray-600 fill-primaryRed"
                viewBox="0 0 100 101"
                fill="none"
                ...
                >
                  <div className="flex flex-col gap-8 mt-8 ">
                    {data.length !== 0 ? (
                      ...
                      <p className="text-center text-3xl text-gray-500 font-semibold">
                        0 Item Found
                      </p>
                    )}
                  </div>
