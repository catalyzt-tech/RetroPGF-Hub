...
const Navbar = ({
  ...
  async function SubmitLogout(
    ...
    if (res.status === 200) {
      ...
      toast.error(
        res.error?.data.msg! || 'Something went wrong when try to logout'
      )
      setLoading(false)
    }
    signOut(getAuth(app))
  ...
  return (
    <>
      ...
      <div
        className={`${bgColor} ${
          shadow && 'shadow'
        } relative z-40 flex items-center lg:justify-between justify-between px-4 md:px-8 py-2 lg:px-12 lg:py-4 h-[4.5rem] w-full`}
      >
            ...
            <Menu as="div" className="relative inline-block text-left">
              ...
              >
                <Menu.Items className="absolute p-4 pt-6 z-20 right-0 mt-2 w-80 origin-top-right flex flex-col gap-2  rounded-md bg-white shadow-lg ring-1 ring-black/5 focus:outline-none">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div className="flex flex-col gap-1 overflow-hidden">
                      <p className="tex-sm font-semibold text-gray-800 line-clamp-1">
                        {globalState.user.username}
                      </p>
                      <h6 className="text-sm font-normal text-gray-600 line-clamp-1">
                ...
                <p className="text-sm font-normal">Account</p>
                    ...
                    <a
                      href={
                        'https://github.com/catalyzt-tech/RetroPGF-Hub/issues'
                      }
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex gap-1 p-2 hover:bg-gray-100 rounded-lg cursor-pointer"
                    >
                      <Chat size={20} className="fill-gray-800" />
                    ...
                    >
                      <Help size={20} className="fill-gray-800" />
                  ...
                  <button
                    ...
                    className="flex gap-1 p-2 hover:bg-gray-100 rounded-lg"
                  >
                    <Logout size={20} className="fill-gray-800" />
